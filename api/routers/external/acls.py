...
class ACLs:
    ...
    def get_stock(symbol):
        ...
        url = f"""{ALPHAVANTAGE_SERVICE}
            ...
            &interval=15min&apikey={ALPHAVANTAGE_API_KEY}"""
        response = requests.get(url)
        content = json.loads(response.content)
        stock = {}
        try:
    ...
    def search_all_stocks(value):
        ...
        url = f"""{FMP_SERVICE}
            /api/v3/search?query={value}&limit=10&apikey=
            {FMP_API_KEY}"""
        response = requests.get(url)
        content = json.loads(response.content)
        stocks = []
    ...
    def get_all_news_items():
        ...
        url = f"""{ALPHAVANTAGE_SERVICE}/
            query?function=NEWS_SENTIMENT&topics=technology&apikey=
